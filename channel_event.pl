...
binmode STDOUT, ':encoding(UTF-8)'
	or print {*STDERR} "Failed to set binmode on STDOUT, Error $ERRNO\n";

my ( $nick, $channel, $event, $bot_username ) = @ARGV;
if (   !defined $nick
	|| !defined $channel
	|| !defined $event
	|| !defined $bot_username )
{
	print {*STDERR} "Usage: channel_event.pl nick channel event bot_username\n";
	exit 1;
}
elsif ( $event ne 'chanjoin'
	and $event ne 'chanpart'
	and $event ne 'chansaid' )
{
	print {*STDERR}
		"Unknown event!  I only know about chanjoin, chanpart and chansaid\n";
	exit 1;
}
my $is_in_file         = 0;
...
if ( $event_file_exists == 1 ) {
	...
	binmode $event_read_fh, ':encoding(UTF-8)'
		or print {*STDERR} 'Failed to set binmode on $event_read_fh, Error'
		. "$ERRNO\n";
	my @event_array = <$event_read_fh>;
	my ($event_who_update,  $event_spoke_update,
		$event_join_update, $event_part_update
	);
...
# If the event file hasn't been created yet we need to create it
	...
	print {*STDERR}
		"Event file $channel_event_file does not exist yet, creating\n";
	...
	push @event_after_array,
		"<$nick> $event_spoke_new $event_join_new $event_part_new";
}
...
if ( $is_in_file == 0 and $event_file_exists == 1 ) {
	print {*STDERR} "Haven't seen \"$nick\" before, adding\n";
	...
	push @event_after_array,
		"<$nick> $event_spoke_add $event_join_add $event_part_add";
