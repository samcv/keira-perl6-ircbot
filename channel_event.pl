...
if ( !defined $nick || !defined $channel || !defined $event || !defined $bot_username ) {
	...
	print {*STDERR} "Unknown event!  I only know about chanjoin, chanpart and chansaid\n";
	exit 1;
}
my $is_in_file         = 0;
...
if ( $event_file_exists == 1 ) {
	...
	foreach my $line (@event_array) {
		...
		if ( $nick =~ /^$event_who_file?.?/i ) {
			...
			push @event_after_array, "<$event_who_update> $event_spoke_update $event_join_update $event_part_update";
		}
...
# If the event file hasn't been created yet we need to create it
	...
	print {*STDERR} "Event file $channel_event_file does not exist yet, creating\n";
